...
Cookbook for running patchium in production-shaped workflows. From real dogfood runs (geminixprize 5-thread fan-out, iterative single-session, mixed-tool 12-angle competitor scan).

For first-install / agent contract see [AGENTS.md](../AGENTS.md). This doc is for operators who already run patchium.
...
| Pure patchium, single session iterative | **53 min** | Drowned operator in `start && go && text` loops for queries WebSearch could've answered |
| Pure patchium, 5-thread fan-out | 10 min | Right for breadth, wrong when threads build on each other |
...
Set at daemon-bootstrap (`patchium daemon start`) or in the daemon's env. None required.

| Variable | Default | When to set |
|---|---|---|
| `PATCHIUM_MAX_SESSIONS` | `4` | Fan-out runs with ≥5 parallel sessions |
| `PATCHIUM_DEFAULT_HEADLESS` | unset | Fan-out / background — keeps Chrome off your desktop |
| `PATCHIUM_DEFAULT_SAFETY` | `flag-only` | `off` to skip prompt-injection scan; `wrap`/`redact` for higher stakes |
| `PATCHIUM_LOG_VERBS` | `0` | `1` for full per-verb audit trail (pair with `PATCHIUM_LOG_LEVEL=DEBUG`) |
| `PATCHIUM_WARM` | `both` | `eager`/`opportunistic`/`both`/`off` — pre-warm pool behavior |
| `PATCHIUM_WARM_RECYCLE` | `0` | `1` for "close/reopen same session" workflows |
| `PATCHIUM_SECRETS_KEY` | unset | Base64 32-byte vault key when not using OS keyring |
| `PATCHIUM_DISABLE_SANDBOX` | `0` | Last-resort sandbox opt-out (Docker w/o user namespaces only) |

Bootstrap for fan-out research:

```
patchium daemon start --max-sessions 8 --default-headless --default-safety wrap --log-verbs
```

8 parallel slots, headless, prompt-injection wrapping, full audit trail in `$XDG_RUNTIME_DIR/patchium/daemon.log`.
...
for url in <candidates>; do patchium verify_url --url "$url"; done   # kill dead candidates
patchium research \
  --target https://<surviving>.com \
  --intent "<q1>" --intent "<q2>" --intent "<q3>" \
  --output-dir ./research-out
patchium logs --since 10m --tail 50    # watch live (separate terminal)
```
...
patchium --session deepdive start --headless
...
patchium logs --session deepdive --tail 100 --errors-only   # post-mortem
```

## Recipe 3: Mixed-tool fan-out (the recommended default)

The pattern that delivered 13× throughput. WebSearch for breadth, patchium for what only a real browser can do.
...
3. Dispatch subagent with ONLY validated URLs + patchium-shaped sub-tasks

Subagent brief routes per-task:
- WebSearch for: Google searches, news lookup, Reddit threads
- patchium for: marketplaces (Xero App Store), live competitor domains, gov SPAs
...
**2. Headed mode in fan-out** — desktop clutter + repeated operator corrections. Either pass `headless: true` on every call or set `PATCHIUM_DEFAULT_HEADLESS=1` once at bootstrap.

**3. Sequential single-session for breadth questions** — 53 min vs 10 min for comparable scope. When sub-questions are independent, `patchium research` instead of `session_new && start && go && text` loops.

**4. Patchium for what WebSearch does well** — ~3× wall clock. Patchium's edge: SPA execution, walled-page bypass, live-domain probing, audit trails. ~70% of iterative research is WebSearch territory.
...
patchium logs --session <name> --tail 200       # needs PATCHIUM_LOG_VERBS=1 at bootstrap
patchium session prune --pattern <prefix> --dry-run
patchium session prune --pattern <prefix>
```
...
- **Search index lag** — WebSearch can hide newly-launched competitors. Second-pass via patchium against the actual marketplace.
