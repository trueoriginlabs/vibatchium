"""Wave 7.7.4 — PATCHIUM_DEFAULT_HEADLESS env var.
...
PATCHIUM_DEFAULT_HEADLESS=1 (or set via `patchium daemon start
...
def test_default_headless_resolution_logic(monkeypatch):
    ...
    def _resolve(args: dict) -> bool:
        ...
        env = os.environ.get("PATCHIUM_DEFAULT_HEADLESS", "0").lower()
        return env in ("1", "true", "yes", "on")

    # No env, no arg → headed (preserves existing default)
    monkeypatch.delenv("PATCHIUM_DEFAULT_HEADLESS", raising=False)
    assert _resolve({}) is False

    # Env set, no arg → headless
    monkeypatch.setenv("PATCHIUM_DEFAULT_HEADLESS", "1")
...
def test_explicit_args_always_win_over_env_default(monkeypatch):
    ...
    def _resolve(args):
        ...
        if "headless" in args:
            return bool(args["headless"])
        return os.environ.get("PATCHIUM_DEFAULT_HEADLESS", "0").lower() in (
            "1", "true", "yes", "on"
        )
    assert _resolve({"headless": False}) is False
    assert _resolve({"headless": True}) is True
